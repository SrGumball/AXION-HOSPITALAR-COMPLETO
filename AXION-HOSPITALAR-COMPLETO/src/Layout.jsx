...
const sateliteMenu = [
  ...
  { name: "Folha de Reg. Específico", icon: Book, page: "SatelitePortaria" },
  { name: "Estoque Atual", icon: Package, page: "SateliteEstoque" },
];
...
const carrinhoMenu = [
  { name: "Carrinho de Parada", icon: Activity, page: "CarrinhoParada" },
];

function getMenuForPage(pageName) {
  if (pageName === "CarrinhoParada") return { menu: carrinhoMenu, subtitle: "Carrinho de Parada" };
...
function LicenseIndicator() {
  ...
  return (
    <button 
      onClick={openModal}
      className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-white/5 w-full transition-colors text-left"
    >
      <div className={cn("w-2.5 h-2.5 rounded-full", valid ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)] animate-pulse")} />
...
export default function Layout({ children, currentPageName }) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 transition-colors">
      ...
      {/* Mobile Header */}
      <div className="lg:hidden print:hidden fixed top-0 left-0 right-0 h-16 bg-[#0F172A] z-40 px-4 flex items-center justify-between shadow-lg">
      ...
      {sidebarOpen && (
        <div
          className="lg:hidden print:hidden fixed inset-0 bg-black/60 z-40 backdrop-blur-sm"
      ...
      {/* Sidebar — navy escuro */}
      <aside className={cn(
        "fixed top-0 left-0 h-full w-56 bg-[#0F172A] z-50 transition-transform duration-300 flex flex-col print:hidden",
        "lg:translate-x-0",
      ...
      {/* Main Content */}
      <main className="lg:ml-56 min-h-screen pt-16 lg:pt-0 print:ml-0 print:pt-0 print:min-h-0">
        {children}
      </main>
    </div>
  );
