...
async function performInitialLoad() {
  ...
  for (let i = 0; i < ENTITIES.length; i++) {
    ...
    try {
      // Supabase: buscar todos os registros da tabela
      const { data: records, error } = await supabase.from(entityName).select('*');
...
async function pullIncrementalFromFirebase(since) {
  ...
  for (const entityName of ENTITIES) {
    try {
      // Supabase: buscar registros atualizados recentemente
...
async function pushPendingToFirebase() {
  ...
  let batchCount = 0;
  const postOps = [];
  // O Supabase não tem o mesmo objeto writeBatch global. Vamos agrupar as operações por tabela
  // e executar bulk upsert ou deletes agrupados por entidade.
  const batchOps = {};
  
  const flushBatch = async () => {
    if (Object.keys(batchOps).length === 0) return;
    
    for (const [entity, ops] of Object.entries(batchOps)) {
       if (ops.deletes.length > 0) {
           const { error } = await supabase.from(entity).delete().in('id', ops.deletes);
           if (error) throw new Error(`Delete failed on ${entity}: ${error.message}`);
       }
       if (ops.upserts.length > 0) {
           const { error } = await supabase.from(entity).upsert(ops.upserts, { onConflict: 'id' });
           if (error) throw new Error(`Upsert failed on ${entity}: ${error.message}`);
       }
    }
    ...
    // Limpa o objeto após flush
    for (const key in batchOps) delete batchOps[key];
    batchCount = 0;
    await pause(60);
  };
  ...
  for (const p of pending) {
    ...
    try {
      // Configura os arrays do batchOps se não existirem
      if (!batchOps[entityName]) {
          batchOps[entityName] = { deletes: [], upserts: [] };
      }

      if (operation === "delete") {
        batchOps[entityName].deletes.push(recordId);
        batchCount++;
      } else {
        ...
        batchOps[entityName].upserts.push(firestoreData);
        batchCount++;
      }
