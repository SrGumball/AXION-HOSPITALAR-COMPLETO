...
import { Bed, Users, Clock, LogOut, CheckCircle2 } from "lucide-react";
...
export default function Enfermagem() {
  ...
  useEffect(() => {
    loadData();
    // Listen for storage changes if multiple windows are open
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Painel de Gestão de Leitos e Admissões
            </p>
          </div>
        </div>
        <Button 
                ...
                >
                  ...
                  {leito.status === 'Ocupado' && leito.paciente ? (
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{leito.paciente.status}</p>
                      <Button 
                        variant="outline" 
                        size="sm" 
                        className="w-full mt-3 h-7 text-xs border-blue-300 text-blue-700 hover:bg-blue-100"
                        onClick={() => handleFreeBed(leito.id)}
                      >
                        Alta / Liberar Leito
                      </Button>
                    </div>
